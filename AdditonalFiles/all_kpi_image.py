...
def create_total_kpi():
    ...
    trgdf = pd.read_sql_query("""
            select sum(amount) as brand_target from V_FF_Brand_TargetAB
                where brand in ( 'OSTOCAL', 'SOLBION', 'XINC', 'LUMONA', 'ROXIM',
                                    'KEFUCLAV', 'DEFCORT') """, conn.azure)
    ...
    salesdf = pd.read_sql_query(""" DECLARE @LastDay as CHAR(8) = CONVERT(VARCHAR(8), GETDATE(), 112)-1
        ...
        select sum(extinvmisc) as brandsales from V_FF_Brand_SalesAB where left(transdate,6)=@This_month and  TRANSDATE<=@LastDay and 
        brand in ( 'OSTOCAL', 'SOLBION', 'XINC', 'LUMONA', 'ROXIM',
                        'KEFUCLAV', 'DEFCORT')  """, conn.azure)
...
def all_kpi_images():
    ...
    LUMONA_achiv = sales_achiv.LUMONA.tolist()[0]
    ROXIM_achiv = sales_achiv.ROXIM.tolist()[0]
    KEFUCLAV_achiv = sales_achiv.KEFUCLAV.tolist()[0]
    DEFCORT_achiv = sales_achiv.DEFCORT.tolist()[0]

    img_draw.text((265, 5), 'up to (' + str(day) + ')', (255, 255, 255), font)
    img_draw.text((45, 75), number_decorator(OSTOCAL_achiv) + '%', (0, 0, 0), font1)
    img_draw.text((210, 75), number_decorator(SOLBION_achiv) + '%', (0, 0, 0), font1)
    img_draw.text((365, 75), number_decorator(XINC_achiv) + '%', (0, 0, 0), font1)
    img_draw.text((520, 75), number_decorator(LUMONA_achiv) + '%', (0, 0, 0), font1)
    img_draw.text((680, 75), number_decorator(ROXIM_achiv) + '%', (0, 0, 0), font1)
    img_draw.text((845, 75), number_decorator(KEFUCLAV_achiv) + '%', (0, 0, 0), font1)
    img_draw.text((1030, 75), number_decorator(DEFCORT_achiv) + '%', (0, 0, 0), font1)
    ...
    LUMONA_trend = sales_trend.LUMONA.tolist()[0]
    ROXIM_trend = sales_trend.ROXIM.tolist()[0]
    KEFUCLAV_trend = sales_trend.KEFUCLAV.tolist()[0]
    DEFCORT_trend = sales_trend.DEFCORT.tolist()[0]

    img_draw.text((180, 120), 'up to (' + str(day) + ')', (255, 255, 255), font)
    img_draw.text((45, 190), number_decorator(OSTOCAL_trend) + '%', (0, 0, 0), font1)
    img_draw.text((210, 190), number_decorator(SOLBION_trend) + '%', (0, 0, 0), font1)
    img_draw.text((360, 190), number_decorator(XINC_trend) + '%', (0, 0, 0), font1)
    img_draw.text((525, 190), number_decorator(LUMONA_trend) + '%', (0, 0, 0), font1)
    img_draw.text((680, 190), number_decorator(ROXIM_trend) + '%', (0, 0, 0), font1)
    img_draw.text((845, 190), number_decorator(KEFUCLAV_trend) + '%', (0, 0, 0), font1)
